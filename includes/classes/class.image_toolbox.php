...
*/
	...
	{	// THIS CREATES A FILE FROM AN IMAGE STRING, SPECIFICALLY FOR LOL TORNEOS
		if	(	!@file_exists
				(	$filename
				)
			):
			$dummy = imagecreatefromstring
			(	$filename
			);			
		...
		switch ($filedata['type']) {
			case 1:
				if(empty($dummy))$dummy = imagecreatefromgif($filename);
				$functionname = $this->_imagecreatefunction;
			...
			case 3:
				if(empty($dummy))$dummy = imagecreatefrompng($filename);
     ...
     * @return bool true on success, otherwise false
     */
	function save($filename = false, $output_type = false, $output_quality = false, $dither = true, $s3 = false) {
		if ($output_type === false) {
			$output_type = $this->_img['main']['output_type'];
		}
		switch ($output_type) {
			...
			case 'PNG8':
				if	(	$this->_types[3]['supported']	<	2
					...
					(	$this->_error_prefix
						.	'Imagetype ('
						.	$this->_types[$output_type]['ext']
						.	') not supported for creating/writing.'
					,	E_USER_ERROR
					);
					return null;
				endif;
				...
				if	(	$this->_gd_version	>=	2
					...
					if	(	$this->_img['main']['indexedcolors']	==	0
						...
						(	$this->_img['main']['width']
						,	$this->_img['main']['height']
						);
						imagecopy
						(	$dummy
						,	$this->_img['main']['resource']
			...
			default:
				trigger_error
				(	$this->_error_prefix
					.	'Output-Imagetype not supported.'
				,	E_USER_ERROR
				);
				return null;
		}
	...
	function _iso2uni($isoline) {
		...
		for ($i=0; $i < strlen($isoline); $i++){
			...
			$new = (isset($iso2uni[ord($thischar)])) ? $iso2uni[ord($thischar)] : '';
			$uniline .= ($new != "") ? $new : $thischar;
		}
		return $uniline;
	}

	function textWillFit
	(	$text
	,	$font
	,	$font_size
	,	$frame_width	=	0
	...
	{	$prepath = 
		...
		if	(	empty
			...
			if	(	empty
				...
				$frame_width = $this->_img['main']['width'];
			endif;
		endif;
		return
		(	$textwidth	<=	$frame_width
		);
	}
     ...
     */	
	function addText($text, $font, $size, $color, $x, $y, $angle = 0, $maxwidth = 0) {
		global $HTTP_SERVER_VARS;
		$prepath = '';
/*		
// THIS BORKS UP RETRIEVAL OF THE TTF FONT FILE ON LINUX... NOT NEEDED IN ANY CASE, SUPPLYING FULL PATH
		if	(	substr
				(	$font
				,	0
				,	1
...
/*
		$this->imagettftextoutline($this->_img['main']['resource'], $size, $angle, $x, $y, $this->_hexToPHPColor($color), $prepath . $font, $text, ceil($size/10), '000000');
*/
		return $textwidth;
	}
